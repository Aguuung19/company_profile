...
@section('content')
    <section class="tableListView-setting" data-aos="zoom-in" data-aos-delay="100">
        ...
        <div class="row">
            <div class="col-align-self-center">
                <h1 class="v-header">Master Menu</h1>
                        ...
                        <tbody>
                            @foreach($master_menu as $item)
                                <tr>
                                    ...
                                    <td>{{$item->status}}</td>
                                    <td>
                                        <a href="{{url('master_menu/'.$item->id.'/edit')}}" class="badge text-bg-primary"><i class="bi bi-pencil-square"></i></a>
                                    </td>
                                    <td>
                                        <form action="{{url('master_menu/'.$item->id)}}" method="POST">
                                            @method('delete')
                                            @csrf

                                            <button class="badge text-bg-danger border-0" type="submit" onclick="return confirm('Delete Menu?')"><i class="bi bi-trash"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
    ...
    <div class="offcanvas offcanfas-lg offcanvas-bottom" tabindex="-1" id="offCanvasAdd" aria-labelledby="offCanvasAddLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title header" id="offcanvasRightLabel">Add Master Menu</h5>
            <button type="submit" class="btn-close" data-bs-dismiss="offcanvas" aria-label="CLose"></button>
        </div>
        <div class="offcanvas-body">
            @include('admin.Master-Menu.add')
        </div>
    </div>
